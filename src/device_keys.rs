...
pub async fn read_or_create(path: impl AsRef<Path>) -> Result<ssh_key::PublicKey> {
    let path = path.as_ref();
    let key_path = tokio::fs::metadata(path).await;

    if let Err(err) = key_path {
        if err.kind() == ErrorKind::NotFound {
            warn!("Could not read private key, generating a new key");
            let mut unencrypted_key = ssh_key::PrivateKey::random(&mut ssh_key::rand_core::OsRng, Algorithm::Ed25519)?;
            ...
            let public_key = unencrypted_key.public_key();

            Ok(public_key.clone())
        } else {
            let err: Result<ssh_key::PublicKey> = Err(err.into());
...
mod test {
    ...
    async fn test_reads_key_if_exists() {
        ...
        let path = file.path().to_owned();
        let key = new_key();
        key.write_openssh_file(&path, ssh_key::LineEnding::LF).unwrap();

        let read_key = read_or_create(&path).await.unwrap();
